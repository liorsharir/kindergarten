    ...
    `,(answer)=>{
        if(answer){
            ...
            let sendToServer={
                summery : title,
                body:body,
                start:  mon.allDate[day-1]+'T06:00:00',      
                end:    mon.allDate[day-1]+'T20:00:00'           
            }
...
function getMessageForMonth(){
    ...
    if(data.events)
        data.events.forEach(e=>{
            if( mon.areDateInSameMonth(e.start.fullYear)  ){
                message[Number.parseInt(e.start.day)+1] = renderEventCalendar(e)
            }
        })
    return message
}
